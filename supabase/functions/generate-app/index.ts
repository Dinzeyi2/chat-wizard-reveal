...
function createMockResponse(prompt: string, projectName: string): any {
  const projectId = crypto.randomUUID();
  
  return {
    projectId,
    projectName: projectName || "MockProject",
    description: `A mock project based on: "${prompt.substring(0, 100)}..."`,
    files: [
      {
        path: "src/App.tsx",
        content: `
  ...
  return (
    <div className="App">
      ...
      <main className="p-4">
        ...
        <p>This is a mock app created for development purposes.</p>
        <p>Your prompt was: "${prompt.substring(0, 100)}..."</p>
      </main>
    </div>
  );
}
...
ReactDOM.createRoot(document.getElementById('root')!).render(
  ...
  "dependencies": {
    ...
    challenges: [
      {
        id: "mock-challenge-1",
        title: "Add a Navigation Component",
        description: "Create a navigation component with links to Home, About, and Contact pages."
      },
      {
        id: "mock-challenge-2",
        title: "Implement Dark Mode Toggle",
        description: "Add a button that toggles between light and dark mode for the application."
      }
    ],
    explanation: "This is a mock project generated for development purposes. In production, this would be generated by AI based on your prompt."
  };
}
...
serve(async (req) => {
  ...
  try {
    ...
    const { prompt, projectName, completionLevel = 'intermediate', model = 'gemini-1.5-pro', mockResponse = false } = requestData as AppGenerationRequest;
    ...
    // If mockResponse is true, immediately return a mock response
    if (mockResponse) {
      console.log("Generating mock response as requested");
      const mockData = createMockResponse(prompt, projectName);
    ...
    if (!geminiApiKey) {
      ...
      // Instead of failing, fall back to a mock response
      const mockData = createMockResponse(prompt, projectName);
      return new Response(JSON.stringify(mockData), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" }
    ...
    console.log(`Calling Gemini API (model: ${model}) to generate application with intentional challenges based on Perplexity design`);
    
    try {
      // Use the specified model (defaulting to gemini-1.5-pro)
    ...
    // Even if there's a general error, provide a mock response
    const mockData = createMockResponse("Error occurred, generating mock response", "MockProject");
    return new Response(JSON.stringify(mockData), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" }
    });
